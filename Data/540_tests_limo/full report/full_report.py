...
for bagFile in listOfBagFiles:
	...
	for i in range(len(X_vic)):
		if (X_vic[i]>X_vic_ini):
			...
			#Compute speed
			t_now = (int(str(t_vic[i]))-int(str(time_trigger)))*0.000000001
			print(t_now)
			
			dt = (t_now -t_last)
			rt.append(t_now)
			real_time.append(rt)
			speed_x = (X_v_real[z]-X_v_real_last)/dt
			speed_y = (Y_v_real[z]-Y_v_real_last)/dt
			speed.append(np.sqrt(speed_x**2+speed_y**2))

			#Memory
			t_last = rt[z]
			X_v_real_last = X_v_real[z]
			Y_v_real_last = Y_v_real[z]
			z = z+1
			
	X_list_v.append(X_v_real)
	Y_list_v.append(Y_v_real)
	speed_list.append(speed)
	t_list.append(real_time)
...
for i in range(len(t_list)):
	...
	plt.plot(t_list[i],speed_list[i],'-b', label='Vitesse inertielle')
	plt.title("Vitesse inertielle selon Vicon")
	plt.xlabel('Temps (s)')
