...
Ca_f = Fn_f/0.5
slip_x_star_f = mu*Fn_f/(8*Cx_f**2)*(mu*Fn_f+4*Cx_f+np.sqrt((mu*Fn_f)**2+8*mu*Fn_f*Cx_f))
slip_a_star_f = mu*Fn_f/(2*Ca_f)
        
Cx_r = Fn_r*mu/slip_max
Ca_r = Fn_r/0.5
slip_x_star_r = mu*Fn_r/(8*Cx_r**2)*(mu*Fn_r+4*Cx_r+np.sqrt((mu*Fn_r)**2+8*mu*Fn_r*Cx_r))
slip_a_star_r = mu*Fn_r/(2*Ca_r)
...
while (v_x>0):
    ...
    # Compute forces for front wheels
    Cx_star_f = (4*np.sqrt(Cx_f**2*slip_x_star_f**2+Ca_f**2*np.tan(slip_a_f)**2)-(1-slip_x_star_f)*mu*Fn_f)/(4*(Cx_f**2*slip_x_star_f**2+Ca_f**2*np.tan(slip_a_f)**2))*mu*Fn_f*Cx_f
    Ca_star_f = (4*np.sqrt(Cx_f**2*slip_x_f**2+Ca_f**2*np.tan(slip_a_star_f)**2)-(1-slip_x_f)*mu*Fn_f)/(4*(Cx_f**2*slip_x_f**2+Ca_f**2*slip_a_star_f**2))*mu*Fn_f*Ca_f
    
    
    Fx_f = Cx_star_f*slip_x_f
    Fy_f = Ca_star_f*slip_a_f
#    print "Fx_f: ", Fx_f
#    print "Fy_f: ", Fy_f
#
#    print(np.sqrt((mu*Fn_f)**2-Fy_f**2))
#     
#    if (abs(Fx_f)>np.sqrt((mu*Fn_f)**2-Fy_f**2)):
#        if (Fx_f>=0):
#            Fx_f = np.sqrt((mu*Fn_f)**2-Fy_f**2)
#        else:
#            Fx_f = -np.sqrt((mu*Fn_f)**2-Fy_f**2)            
#    if (abs(Fy_f)>np.sqrt((mu*Fn_f)**2-Fx_f**2)):
#        if (Fy_f>=0):
#            Fy_f = np.sqrt((mu*Fn_f)**2-Fx_f**2)
#        else:
#            Fy_f = -np.sqrt((mu*Fn_f)**2-Fx_f**2)
                       
    # Compute forces for rear wheels
    Cx_star_r = (4*np.sqrt(Cx_r**2*slip_x_star_r**2+Ca_r**2*np.tan(slip_a_r)**2)-(1-slip_x_star_r)*mu*Fn_r)/(4*(Cx_r**2*slip_x_star_r**2+Ca_r**2*np.tan(slip_a_r)**2))*mu*Fn_r*Cx_r
    Ca_star_r = (4*np.sqrt(Cx_r**2*slip_x_r**2+Ca_r**2*np.tan(slip_a_star_r)**2)-(1-slip_x_r)*mu*Fn_r)/(4*(Cx_r**2*slip_x_r**2+Ca_r**2*slip_a_star_r**2))*mu*Fn_r*Ca_r
        
    Fx_r = Cx_star_r*slip_x_r
    Fy_r = Ca_star_r*slip_a_r
        
    if (abs(Fx_r)>np.sqrt((mu*Fn_r)**2+(-Fy_r)**2)):
        if (Fx_r>=0):
            Fx_r = np.sqrt((mu*Fn_r)**2+(-Fy_r)**2)
        else:
            Fx_r = -np.sqrt((mu*Fn_r)**2+(-Fy_r)**2)            
    if (abs(Fy_r)>np.sqrt((mu*Fn_r)**2+(-Fx_r)**2)):
        if (Fy_r>=0):
            Fy_r = np.sqrt((mu*Fn_r)**2+(-Fx_r)**2)
        else:
            Fy_r = -np.sqrt((mu*Fn_r)**2+(-Fx_r)**2)
