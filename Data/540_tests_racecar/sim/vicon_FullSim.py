...
# Open workbook
workbook = xlrd.open_workbook('/home/ubuntu/Desktop/racecar_tests_sim.xlsx')
# Choose sheet
...
Iz = 1.00/12.00*m*((f_length+b_length)**2+width**2)
...
""" 
    ...
    ***** DEPENDS DU MODELE DE TIRE COMPLET EX: S=omega/v_x et F_x,F_y = f(F_z,alpha,S et mu) *****
    ...
    Where 
    F_xf is the longitudinal force applied at the front wheel (N)
    F_xr is the longitudinal force applied at the rear wheel (N)
...
while (v[k] > 0):
	...
	#Compute slip ratio
	if (v[k]>0.05):
		slip.append((omega_r[k+1]-v[k])/v[k])
	else:
		slip.append(0)
