...
wb = xlsxwriter.Workbook('/home/ubuntu/catkin_ws/src/research_racecar/bagfiles/data_excel/vehicule1_sigma_FULL_1000.xlsx')
...
for bagFile in listOfBagFiles:
    ...
    for j in range(nbrOfSimPerBag):
		...
		a_ness         = v_i**2/(2*obs_x)   #equation cinematique
		a_mu = ((m/(cg+1))*mu*g)/m          #Force freinage maximal sur ce sol
		
		# a retravailler
		a_decel = u*0.1*g                   # Deceleration aux roues de la manoeuvre
		if (a_decel > a_mu):
			a_adm = a_mu              
		else:
			a_adm = a_decel
