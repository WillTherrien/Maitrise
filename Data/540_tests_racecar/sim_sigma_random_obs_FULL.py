...
wb = xlsxwriter.Workbook('/home/ubuntu/catkin_ws/src/research_racecar/bagfiles/data_excel/vehicule1_sigma_FULL_2000.xlsx')
...
for bagFile in listOfBagFiles:
    ...
    for j in range(nbrOfSimPerBag):
		...
		obs_x = (random.randrange(1000,20000,1)*0.0001)
		print(obs_x)
		...
		for i in range(len(X)):
			...
			nbre_point = obs_y/sigma_y

			# Check if obstacle is inside ellipses
			if ((((distance_obs-P1_x)**2)/(sigma_x**2)+((obs_y-P1_y)**2)/(sigma_y**2)) <= 1):
				success = 0
			elif ((((distance_obs-P2_x)**2)/(sigma_x**2)+((obs_y-P2_y)**2)/(sigma_y**2)) <= 1):
				success = 0
			elif ((((distance_obs-P3_x)**2)/(sigma_x**2)+((obs_y-P3_y)**2)/(sigma_y**2)) <= 1):
				success = 0
			elif ((((distance_obs-P4_x)**2)/(sigma_x**2)+((obs_y-P4_y)**2)/(sigma_y**2)) <= 1):
				success = 0
			if ((((distance_obs-X[i])**2)/((f_length*np.cos(theta[i])+sigma_x)**2)+((obs_y-Y[i])**2)/((width/2*np.sin(theta[i])+sigma_y)**2)) <= 1):
				success = 0
			else:
				pass
		print(success)
