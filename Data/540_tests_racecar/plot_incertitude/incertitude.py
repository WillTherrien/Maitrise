...
for i in range(len(X_list_v)):
	...
	u = X_list_v[i][list_l] #x-position of the center
	v = Y_list_v[i][list_l]   #y-position of the center
	a = 0.06     #radius on the x-axis (sigma)
	b = 0.004   #radius on the y-axis
	t = np.linspace(0, 2*np.pi, 100)
	plt.plot( u+a*np.cos(t) , v+b*np.sin(t),'-g')
	a2 = 2*0.06     #radius on the x-axis (sigma)
	b2 = 2*0.004   #radius on the y-axis
	plt.plot( u+a2*np.cos(t) , v+b2*np.sin(t),'-y')
	a3 = 3*0.06     #radius on the x-axis (sigma)
	b3 = 3*0.004   #radius on the y-axis
	plt.plot( u+a3*np.cos(t) , v+b3*np.sin(t),'-r')
	plt.plot(X_list_v[i][list_l],Y_list_v[i][list_l],'or')
	plt.plot(X_list_v[i],Y_list_v[i],'--b', label=str(bagNumber_list[i]))
	plt.title("All paths")
